...
def draw_textured_rect(surf_arr, tex_arr, verts):
    tex_w, tex_h = tex_arr.shape[1]/256, tex_arr.shape[0]/256
    min_y = int(min(y for (x,y),_ in verts))
    max_y = int(max(y for (x,y),_ in verts))
    ...
    for y in range(min_y, max_y):
        ...
        w = x2 - x1
        if w <= 0: continue
        du = (u2 - u1) / w * tex_w; dv = (v2 - v1) / w * tex_h
        u1 *= tex_w; v1 *= tex_h
        # スパン描画
        for x in range(int(x1), int(x1)+int(w)):
            surf_arr[x, y] = tex_arr[int(u1), int(v1)]
            u1 += du; v1 += dv

def fill_rect(arr,tex,x,y,w,h,rot=0):
    w = w/2
    h = h/2
    rot = rot * math.pi / 128
    cos = math.cos(rot)
    ...
    ps = [((-wcos - hsin + x), ( wsin - hcos + y)),
          ...
          (( wcos + hsin + x), (-wsin + hcos + y)),
          ((-wcos + hsin + x), ( wsin + hcos + y))]
    draw_textured_rect(arr,tex,[(ps[0],(0,0)),(ps[1],(255,0)),(ps[2],(255,255)),(ps[3],(0,255))])
...
while running:
    ...
    fill_rect(arr,tarr,64,64,128,128,rot)
    del arr
    rot = (rot + 1) & 255
