    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
        suf.fill((0,0,64))
        draw_rotated_texture(suf, tex, cx, cy, r, w, h)
        r += 0.05
...
def draw_rotated_texture(suf, tex, cx, cy, r, w, h):
    tcx = tex.get_width()//2
    ...
    sx = tcx; sy = tcy; cx = bkcx
    for y in range(cy,h):
        pixel_drawn = False  # その行はスプライトのピクセルが描画されたかを記録するフラグ
